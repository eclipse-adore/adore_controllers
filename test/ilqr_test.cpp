...
TEST( ILQRController, ProducesFiniteCommandOnStraightTrajectory )
{
  ...
  Trajectory traj;
  traj.label = "ilqr_straight";

  const double dt    = 0.1;
  const double v_ref = 5.0;

  // Build a straight reference trajectory long enough for the horizon.
  for( int i = 0; i < 50; ++i )
  {
  ...
  // Exercise the parameter interface (currently relying on defaults).
  ...
  const auto last_traj = controller.get_last_trajectory();
  EXPECT_FALSE( last_traj.states.empty() );
}
